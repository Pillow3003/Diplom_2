...
class TestUserRegistration:
    ...
    def test_create_new_user_positive_check(self, non_registration_user):
        """
        Проверка успешной регистрации нового уникального пользователя.

        :param non_registration_user: фикстура с данными нового пользователя (dict)
        """
        response = user_registration(non_registration_user)
        resp_json = response.json()

        assert response.status_code == 200, f"Ожидался статус 200, получен {response.status_code}"
        assert resp_json.get('success') is True, "Поле 'success' должно быть True"
        assert resp_json.get('user', {}).get('name') == non_registration_user['name'], "Имя пользователя не совпадает"
        assert resp_json.get('user', {}).get('email') == non_registration_user['email'], "Email пользователя не совпадает"
    ...
    def test_create_existed_user_negative_check(self, client):
        """
        Проверка, что регистрация уже существующего пользователя возвращает ошибку.

        :param client:
        """
        response = user_registration(client[0])

        assert response.status_code == 403, f"Ожидался статус 403, получен {response.status_code}"
        assert response.json().get('success') is False, "Поле 'success' должно быть False при ошибке"
        assert response.json().get('message') == DataMessages.NOT_UNIQUE_USER, \
    ...
    def test_create_user_empty_input_negative_check(self, non_registration_user, empty_input):
        """
        ...
        """
        new_user_modified = copy.deepcopy(non_registration_user)
